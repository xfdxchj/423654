...
import org.skepsun.kototoro.backups.external.MihonBackupExportRepository
...
class MihonBackupExportService : BaseBackupRestoreService() {

    override val notificationTag = TAG
    override val isRestoreService = false

    @Inject
    lateinit var repository: MihonBackupExportRepository
    ...
    private fun IntentJobContext.buildNotification(progress: Progress): Notification {
        return NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setContentTitle(getString(R.string.export_mihon_backup))
    ...
    private fun showExportToast(summary: MihonBackupExportSummary) {
        Toast.makeText(
            this,
            getString(R.string.export_mihon_backup_saved, summary.exportedCount),
            Toast.LENGTH_SHORT,
        ).show()
    }
    ...
    private fun IntentJobContext.showExportResultNotification(
        ...
        val shareIntent = ShareCompat.IntentBuilder(this@MihonBackupExportService)
            .setStream(fileUri)
        ...
        val notification = NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setContentTitle(getString(R.string.export_mihon_backup))
            .setContentText(getString(R.string.export_mihon_backup_saved, summary.exportedCount))
            ...
            .setContentIntent(
                PendingIntentCompat.getActivity(
                    applicationContext,
                    0,
                    AppRouter.homeIntent(this@MihonBackupExportService),
                    0,
                    false,
                ),
            )
            .addAction(
                ...
                PendingIntentCompat.getActivity(this@MihonBackupExportService, 0, shareIntent, 0, false),
            )
            .build()
    ...
    companion object {
        private const val TAG = "MIHON_BACKUP_EXPORT"
        private const val FOREGROUND_NOTIFICATION_ID = 41

        @CheckResult
        fun start(context: Context, uri: Uri): Boolean = try {
            val intent = Intent(context, MihonBackupExportService::class.java)
