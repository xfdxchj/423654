...
import org.skepsun.kototoro.backups.external.AniyomiBackupExportRepository
import org.skepsun.kototoro.backups.external.MihonBackupExportSummary
...
class AniyomiBackupExportService : BaseBackupRestoreService() {

    override val notificationTag = TAG
    override val isRestoreService = false

    @Inject
    lateinit var repository: AniyomiBackupExportRepository
    ...
    private fun IntentJobContext.buildNotification(progress: Progress): Notification {
        return NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setContentTitle(getString(R.string.export_aniyomi_backup))
    ...
    private fun showExportToast(summary: MihonBackupExportSummary) {
        Toast.makeText(
            this,
            getString(R.string.export_aniyomi_backup_saved, summary.exportedCount),
            Toast.LENGTH_SHORT,
        ).show()
    }
    ...
    private fun IntentJobContext.showExportResultNotification(
        fileUri: Uri,
        summary: MihonBackupExportSummary,
    ) {
        if (!applicationContext.checkNotificationPermission(CHANNEL_ID)) {
            return
        }
        val shareIntent = ShareCompat.IntentBuilder(this@AniyomiBackupExportService)
            .setStream(fileUri)
        ...
        val notification = NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setContentTitle(getString(R.string.export_aniyomi_backup))
            .setContentText(getString(R.string.export_aniyomi_backup_saved, summary.exportedCount))
            ...
            .setContentIntent(
                PendingIntentCompat.getActivity(
                    applicationContext,
                    0,
                    AppRouter.homeIntent(this@AniyomiBackupExportService),
                    0,
                    false,
                ),
            )
            .addAction(
                ...
                PendingIntentCompat.getActivity(this@AniyomiBackupExportService, 0, shareIntent, 0, false),
            )
            .build()
    ...
    companion object {
        private const val TAG = "ANIYOMI_BACKUP_EXPORT"
        private const val FOREGROUND_NOTIFICATION_ID = 42

        @CheckResult
        fun start(context: Context, uri: Uri): Boolean = try {
            val intent = Intent(context, AniyomiBackupExportService::class.java)
