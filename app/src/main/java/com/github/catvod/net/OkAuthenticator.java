...
public class OkAuthenticator implements Authenticator {
    ...
    public Request authenticate(@Nullable Route route, @NonNull Response response) {
        if (route == null || response.request().header(HttpHeaders.PROXY_AUTHORIZATION) != null) {
            return null;
        }
        if (!(route.proxy().address() instanceof InetSocketAddress)) {
            return null;
        }
        ...
        for (Proxy item : proxy) {
            for (String host : item.getHosts()) {
                ...
                for (String url : item.getUrls()) {
                    ...
                    String userInfo = android.net.Uri.parse(url).getUserInfo();
                    if (userInfo != null) {
                        return response.request().newBuilder()
                            .header(HttpHeaders.PROXY_AUTHORIZATION, Util.basic(userInfo))
                            .build();
                    }
                }
            }
