...
public class ResponseInterceptor implements Interceptor {
    ...
    public Response intercept(@NonNull Chain chain) throws IOException {
        ...
        if ("deflate".equals(response.header(HttpHeaders.CONTENT_ENCODING))) {
            return deflate(response);
        }
        if (response.code() == 406 && redirectMap.containsKey(request.url().toString())) {
            return redirect(request, response);
        }
        if (response.code() == 302 && response.header(HttpHeaders.LOCATION) != null) {
            redirectMap.put(response.header(HttpHeaders.LOCATION), request.url().toString());
        }
        return response;
    }
    ...
    private Request check(Request request) {
        ...
        for (Header item : headers) {
            if (Util.containOrMatch(host, item.getHost())) {
                Json.toMap(item.getHeader()).forEach(builder::header);
            }
        }
    ...
    private Response redirect(Request request, Response response) {
        return new Response.Builder()
            ...
            .code(302)
            .message("Found")
            .header(HttpHeaders.LOCATION, redirectMap.get(request.url().toString()))
            .build();
    }
