...
public final class UriUtil {
    ...
    private static String removeDotSegments(StringBuilder uri, int offset, int limit) {
        ...
        while (index <= limit) {
            ...
            if (index == limit) {
                nextSegmentStart = index;
            } else if (uri.charAt(index) == '/') {
                nextSegmentStart = index + 1;
            } else {
                index++;
                continue;
            }
            if (index == segmentStart + 1 && uri.charAt(segmentStart) == '.') {
                uri.delete(segmentStart, nextSegmentStart);
                limit -= nextSegmentStart - segmentStart;
                index = segmentStart;
            } else if (index == segmentStart + 2 && uri.charAt(segmentStart) == '.' && uri.charAt(segmentStart + 1) == '.') {
                int prevSegmentStart = uri.lastIndexOf("/", segmentStart - 2) + 1;
                int removeFrom = Math.max(prevSegmentStart, offset);
                uri.delete(removeFrom, nextSegmentStart);
                limit -= nextSegmentStart - removeFrom;
                segmentStart = prevSegmentStart;
                index = prevSegmentStart;
            } else {
                index++;
                segmentStart = index;
            }
